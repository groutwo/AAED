...
{   if (this != &C)   // evitar autoasignación
    {// Destruir el vector y crear uno nuevo si es necesario
        if (Lmax != C.Lmax)
            delete[] elementos, Lmax = C.Lmax, elementos = new T[Lmax];
        // Copiar el vector
...
{ assert(!vacia()); return elementos[inicio]; }

template <typename T> inline void Cola<T>::pop()
{ assert(!vacia()); inicio = (inicio + 1) % Lmax; }

template <typename T> inline void Cola<T>::push(const T& x)
{ assert(!llena()); fin = (fin + 1) % Lmax; elementos[fin] = x; }
