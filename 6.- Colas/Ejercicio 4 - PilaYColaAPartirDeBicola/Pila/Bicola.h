...
void Bicola<tEle>::copiar(const Bicola<tEle>& bicola)
{
    if (bicola.inicio)   // bicola no está vacía
   {
    ...
    for (nodo *nodoActual = bicola.inicio->sig; nodoActual!=bicola.fin->sig; nodoActual = nodoActual->sig)
...
{
    while(!bicola.vacia())
        cout<<bicola.topePila()<<" ", bicola.popPila();
    cout<<endl;
}
#endif
